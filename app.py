...
def generate_label(p_name, p_type, pk_num, date_str):
    ...
    draw.text((TOTAL_W/2, SEC_A_H/2), str(p_name).upper(), fill="white", anchor="mm", font=product_font)
    ...
    draw.text((SEC_B_W/2, SEC_A_H + ((TOTAL_H-SEC_A_H-SEC_D_H)/2)), pk_text, fill="black", anchor="mm", font=pk_font)
    ...
    draw.text((TOTAL_W - 40, TOTAL_H - (SEC_D_H/2)), f"Date: {date_str}", fill="black", anchor="rm", font=date_font)
...
if not df.empty:
    ...
    if st.button("🚀 สร้างป้าย"):
        ...
        # --- ส่วนปรับปรุงชื่อไฟล์ตาม Format ที่ต้องการ ---
        formatted_date = dt_val.strftime("%d-%m-%Y")
        custom_filename = f"{row['ProductName']} - PK {pk_no} - {formatted_date}.png"

        st.download_button(
            label="📥 ดาวน์โหลดไฟล์สำหรับพิมพ์",
            data=buf.getvalue(),
